...
typedef struct st_LWSaveState {
	...
	int		    (*depth)    (void *data);
	void		(*writeDP)  (void *data, const double *, int n);
} LWSaveState;
...
typedef struct st_LWLoadState {
	...
	void	(*endBlk)  (void *data);
	int		(*depth)   (void *data);
	int		(*readDP)  (void *data, double *, int n);
} LWLoadState;
