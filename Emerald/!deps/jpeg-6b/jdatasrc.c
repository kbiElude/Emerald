 ...
 * jdatasrc.c
 *
 * Additional modifications: kbi/dimness [2009] - support for jpeg files stored in memory was added.
 * 
...
typedef struct {
  struct jpeg_source_mgr pub;				/* public fields */
  char*                  inptr;				/* source memory stream */
  JOCTET*                buffer;			/* start of buffer */
  int					 nBufferBytes;		/* how many bytes does the buffer consist of? */
} mem_source_mgr;
...
METHODDEF(boolean) mem_fill_input_buffer (j_decompress_ptr cinfo)
{
  ...
  size_t      nbytes = src->nBufferBytes;

  src->pub.next_input_byte = src->inptr;
  src->pub.bytes_in_buffer = nbytes;
...
//-->
GLOBAL(void) jpeg_mem_src (j_decompress_ptr cinfo, void* inBuf, unsigned int inBufLength)
{
	mem_src_ptr src;
