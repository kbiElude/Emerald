...
#define _LOG(level,file,line,text,...)                                                                  \
    ...
    {                                                                                                   \
        ...
        static char log_helper[LOG_MAX_LENGTH];                                                         \
                                                                                                        \
        memset(log_helper, 0, LOG_MAX_LENGTH);                                                          \
        sprintf_s(log_helper, LOG_MAX_LENGTH, "[File %s // line %d]: " text, file, line, __VA_ARGS__);  \
        system_log_write(level, log_helper);                                                            \
                                                                                                        \
        system_critical_section_leave(log_file_handle_cs);                                              \
    }

#define LOG_TRACE(text,...) \
    _LOG(LOGLEVEL_TRACE, __FILE__, __LINE__, text,__VA_ARGS__)
#define LOG_INFO(text,...) \
    _LOG(LOGLEVEL_INFORMATION,__FILE__, __LINE__, text,__VA_ARGS__)
#define LOG_ERROR(text,...) \
    _LOG(LOGLEVEL_ERROR, __FILE__, __LINE__, text,__VA_ARGS__)
#define LOG_FATAL(text,...) \
    _LOG(LOGLEVEL_FATAL, __FILE__, __LINE__, text,__VA_ARGS__)
 ...
 */
EMERALD_API void system_log_write(system_log_priority, const char*);
