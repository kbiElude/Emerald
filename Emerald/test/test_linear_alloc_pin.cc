...
TEST(LinearAllocPinTest, EntryAlignmentTest)
{
    ...
    system_linear_alloc_pin handle = system_linear_alloc_pin_create(allocation_size,
                                                                    n_entries,
                                                                    n_pins);
    {
        for (size_t n_entry = 0;
                    n_entry < 5;
                  ++n_entry)
        {
...
TEST(LinearAllocPinTest, BlobReallocationTest)
{
                                                                    ...
                                                                    n_pins);
    {
        ...
        ASSERT_EQ(entries[4],
                  (char*) entries[0] + sizeof(void*) * 4);
        ASSERT_NE(entries[5],
                  (char*) entries[0] + sizeof(void*) * 5);

        /* Make sure reported pointers are not equal! */
        for (size_t n_base_ptr = 0;
                    n_base_ptr < 10;
                  ++n_base_ptr)
        {
            for (size_t n_base2_ptr = n_base_ptr + 1;
                        n_base2_ptr < 10;
                      ++n_base2_ptr)
            {
                ASSERT_NE(entries[n_base_ptr],
                          entries[n_base2_ptr]);
            }
        }
    }
...
TEST(LinearAllocPinTest, Pins)
{
                                                                    ...
                                                                    n_pins);
    {
                  ...
                  ++n_pass)
        {
            ...
            ASSERT_EQ(system_linear_alloc_pin_get_from_pool(handle),
                      entries[n_pass]);
    ...
    //
    handle = system_linear_alloc_pin_create(allocation_size,
                                            n_entries / 2,
                                            n_pins);
    {
        for (size_t n_pass = 1;
