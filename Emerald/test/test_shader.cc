...
static void _on_render_frame_triangle_test_callback(ogl_context context, uint32_t n_frames_rendered, system_timeline_time frame_time, void*)
{
    ...
    if (triangle_test_is_first_frame)
    {
        ...
        /* Set up the uniforms */
        entry_points->pGLUniformMatrix4fv(triangle_test_projection_matrix_location, 1, false, system_matrix4x4_get_row_major_data(triangle_test_projection_matrix) );
        entry_points->pGLUniformMatrix4fv(triangle_test_view_matrix_location,       1, true,  system_matrix4x4_get_row_major_data(triangle_test_view_matrix) );
        ASSERT_EQ(entry_points->pGLGetError(), GL_NO_ERROR);

        /* Set up buffer object for position data */
        entry_points->pGLGenBuffers(1, &triangle_test_position_bo_id);
        ASSERT_NE(triangle_test_position_bo_id, -1);
        ASSERT_EQ(entry_points->pGLGetError(),  GL_NO_ERROR);

        entry_points->pGLBindBuffer(GL_ARRAY_BUFFER, triangle_test_position_bo_id);
        ASSERT_EQ(entry_points->pGLGetError(), GL_NO_ERROR);
        entry_points->pGLBufferData(GL_ARRAY_BUFFER, sizeof(triangle_test_position_data), triangle_test_position_data, GL_STATIC_DRAW);
        ASSERT_EQ(entry_points->pGLGetError(), GL_NO_ERROR);

        /* Set up VAOs */
        entry_points->pGLGenVertexArrays(1, &triangle_test_position_vao_id);
        ASSERT_NE(triangle_test_position_vao_id, -1);
        ...
        entry_points->pGLVertexAttribPointer    (triangle_test_position_location, 4, GL_FLOAT, 0, 0, 0);

        /* Set up clear color */
        entry_points->pGLClearColor(1.0, 0, 1.0f, 1);
    ...
    entry_points->pGLBindVertexArray(triangle_test_position_vao_id);
    entry_points->pGLDrawArrays(GL_TRIANGLES, 0, 3);
}

static void _on_render_frame_creation_test_callback(ogl_context context, uint32_t n_frames_rendered, system_timeline_time frame_time, void*)
{
    const ogl_context_gl_entrypoints* entry_points = NULL;
...
TEST(ShaderTest, CreationTest)
{
    ...
    ogl_rendering_handler rendering_handler = ogl_rendering_handler_create_with_max_performance_policy(_on_render_frame_creation_test_callback, 0);
    ...
    test_shader = ogl_shader_create(gl_context, SHADER_TYPE_FRAGMENT);
...
TEST(ShaderTest, FullViewportTriangleTest)
{
    /* Create the window */
    const int     xywh[]            = {0, 0, 320, 240};
    system_window window_handle     = system_window_create_not_fullscreen(xywh, system_hashed_ansi_string_create("triangle test"), false, 0, true);
    HWND          window_sys_handle = 0;
    ...
    /* Create a rendering handler */
    ogl_rendering_handler rendering_handler = ogl_rendering_handler_create_with_max_performance_policy(_on_render_frame_triangle_test_callback, 0);
    ASSERT_TRUE(rendering_handler != NULL);
    ...
    test_vertex_shader = ogl_shader_create(gl_context, SHADER_TYPE_VERTEX);
    ...
    ogl_shader test_fragment_shader = ogl_shader_create(gl_context, SHADER_TYPE_FRAGMENT);
    ...
    ASSERT_TRUE(ogl_program_attach_shader(test_program, test_vertex_shader) );
    ASSERT_TRUE(ogl_program_attach_shader(test_program, test_fragment_shader) );
    ...
    ASSERT_TRUE(ogl_program_get_uniform_by_name(test_program, system_hashed_ansi_string_create("view_matrix"),       &view_matrix_uniform_descriptor)       );
    ASSERT_TRUE(ogl_program_get_uniform_by_name(test_program, system_hashed_ansi_string_create("projection_matrix"), &projection_matrix_uniform_descriptor) );
    ...
    ASSERT_NE(triangle_test_projection_matrix_location, -1);
    ASSERT_NE(triangle_test_view_matrix_location,       -1);

    /* Retrieve attribute location */
    const ogl_program_attribute_descriptor* position_attribute_descriptor = NULL;

    ASSERT_TRUE(ogl_program_get_attribute_by_name(test_program, system_hashed_ansi_string_create("position"), &position_attribute_descriptor) );

    triangle_test_position_location = position_attribute_descriptor->location;
    ASSERT_NE(triangle_test_position_location, -1);

    /* Create a projection matrix */
    triangle_test_projection_matrix = system_matrix4x4_create_perspective_projection_matrix(53.0f, 320.0f / 240.0f, 0.05f, 25);
