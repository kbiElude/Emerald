...
typedef struct st_LWValString {
    LWType         type;
    char            *buf; // locale specific encoding
    int              bufLen;
} LWValString;
...
typedef struct st_LWPanChoiceDesc {
    LWType       type;
    const char   **items; // locale specific encoding
    int          vertical;
} LWPanChoiceDesc;

typedef struct st_LWPanTextDesc {
    LWType       type;
    const char   **text; // locale specific encoding
} LWPanTextDesc;
...
typedef struct st_LWPanLWItemDesc {
    ...
    void            *global;
    int              itemType;
    char            **list; // locale specific encoding (dependent on LWItemInfo and LWImageList) (client does not set this)
    int              count;
    int              width;
} LWPanLWItemDesc;
...
typedef struct st_LWPanListBoxDesc {
    ...
    int                      (*countFn)(void *);
    char                    *(*nameFn)(void *, int); // return text in locale specific encoding
} LWPanListBoxDesc;
...
typedef struct st_LWPanMultiListBoxDesc {
    ...
    int                      width,visItems, top;  // width in pixels, height in items, top visible item
    int                      (*countFn)(void *);
    char                    *(*nameFn)(void *, int, int); // item, column // return text in locale specific encoding
...
typedef struct st_LWPanTreeDesc {
    ...
    char                    *(*infoFn)(void *data, void *node, int *nodeFlags); /* return "name" for node, fill 0 flags, or save non-zero flags */ // return text in locale specific encoding
...
typedef struct st_DrawFuncs {
    ...
    int     (*textWidth)(LWPanelID,char *);                         /* text in locale specific encoding */
    void    (*drawText)(LWPanelID,char *,int,int,int);              /* text in locale specific encoding,color,x,y */
...
typedef struct st_LWPanelFuncs {
    LWPanelID               (*create)(char *, void *); // locale specific encoding
    ...
    LWControl              *(*addControl)(LWPanelID pan, char *type, ControlDesc *data, const char *label); // locale specific encoding
...
typedef struct st_LWRasterFuncs {
    ...
    void            (*drawText)(LWRasterID,char *,int,int,int);         /* text in locale specific encoding,color,x,y */
    ...
    void            (*blitRaster)(LWRasterID, int, int, LWRasterID, int,int,int,int);  /* src, srcx, srcy, dest, destx, desty, w, h*/
...
#define PANELMENUFLAG_SELECTED   (1<<0)
#define PANELMENUFLAG_UNSELECTED (1<<1)
#define PANELMENUFLAG_HIGHLIGHT  (1<<2)
#define PANELMENUFLAG_DISABLED   (1<<3)
