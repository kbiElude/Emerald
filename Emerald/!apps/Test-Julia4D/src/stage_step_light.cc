...
/** TODO */
static void _stage_step_light_execute(ogl_context context, system_timeline_time time, void* not_used)
{
    ...
    system_matrix4x4 mvp             = NULL;
          ...
          ogl_flyby_get_view_matrix     (context, _light_view_matrix);
    mvp = system_matrix4x4_create_by_mul(main_get_projection_matrix(), _light_view_matrix);
    ...
    {
        entrypoints->pGLUniform4fv(_light_color_uniform_location, 1, light_color);
    ...
    {
        entrypoints->pGLVertexAttrib3fv(_light_in_position_attribute_location, light_position);

        memcpy(gpu_light_position, light_position, sizeof(float) * 3);
    }

    entrypoints->pGLUniformMatrix4fv(_light_mvp_uniform_location, 1, GL_TRUE, system_matrix4x4_get_row_major_data(mvp) );
    ...
    {
        entrypoints->pGLDrawArrays(GL_POINTS, 0, 1);
    }
...
/* Please see header for specification */
PUBLIC void stage_step_light_init(ogl_context context, ogl_pipeline pipeline, uint32_t stage_id)
{
    /* Link  program */
    shaders_fragment_static            fragment_shader = NULL;
    shaders_vertex_combinedmvp_generic vertex_shader   = NULL;

    _light_program  = ogl_program_create                       (context,                          system_hashed_ansi_string_create("light program") );
    fragment_shader = shaders_fragment_static_create           (context, FRAGMENT_STATIC_UNIFORM, system_hashed_ansi_string_create("light fragment") );
    vertex_shader   = shaders_vertex_combinedmvp_generic_create(context,                          system_hashed_ansi_string_create("light vertex") );

    ogl_program_attach_shader(_light_program, shaders_fragment_static_get_shader           (fragment_shader) );
    ogl_program_attach_shader(_light_program, shaders_vertex_combinedmvp_generic_get_shader(vertex_shader) );
    ...
    ogl_program_get_attribute_by_name(_light_program, system_hashed_ansi_string_create("in_position"), &in_position_attribute_data);
    ogl_program_get_uniform_by_name  (_light_program, system_hashed_ansi_string_create("color"),       &color_uniform_data);
    ogl_program_get_uniform_by_name  (_light_program, system_hashed_ansi_string_create("mvp"),         &mvp_uniform_data);
    ...
    entrypoints->pGLGenVertexArrays(1, &_light_vao_id);

    /* Add ourselves to the pipeline */
    ogl_pipeline_add_stage_step(pipeline, stage_id, system_hashed_ansi_string_create("Light preview"), _stage_step_light_execute, NULL);
