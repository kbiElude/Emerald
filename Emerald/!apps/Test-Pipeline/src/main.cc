...
const char* fragment_shader_modification = "#version 330\n"
                                           ...
                                           "}\n";
/* GL deinitialization */
void _deinit_gl(ogl_context context, void* arg)
{
    ...
    entry_points->pGLDeleteFramebuffers(1, &_fbo_id);
...
/* GL initialization */
void _init_gl(ogl_context context, void* arg)
{
    ...
    entry_points->pGLGenFramebuffers(1, &_fbo_id);
    entry_points->pGLGenVertexArrays(1, &_vao_id);

    tos[0] = ogl_texture_create(context, system_hashed_ansi_string_create("1"));
    tos[1] = ogl_texture_create(context, system_hashed_ansi_string_create("2"));

    _to_1 = tos[0];
    _to_2 = tos[1];

    /* Set up texture storage */
    ASSERT_DEBUG_SYNC(_window_size[0] != 0, "Window width is 0");
    ASSERT_DEBUG_SYNC(_window_size[1] != 0, "Window height is 0");

    dsa_entry_points->pGLTextureStorage2DEXT (_to_1, GL_TEXTURE_2D, 1, GL_RGBA8,             _window_size[0], _window_size[1]);
    dsa_entry_points->pGLTextureStorage2DEXT (_to_2, GL_TEXTURE_2D, 1, GL_DEPTH_COMPONENT32, _window_size[0], _window_size[1]);

    dsa_entry_points->pGLTextureParameteriEXT(_to_1, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    dsa_entry_points->pGLTextureParameteriEXT(_to_2, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);

    /* Set up fbo */
    dsa_entry_points->pGLNamedFramebufferTexture2DEXT(_fbo_id, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _to_1, 0);
    dsa_entry_points->pGLNamedFramebufferTexture2DEXT(_fbo_id, GL_DEPTH_ATTACHMENT,  GL_TEXTURE_2D, _to_2, 0);

    /* Set up programs */
    shaders_vertex_fullscreen general_vs      = shaders_vertex_fullscreen_create(_context, true,                 system_hashed_ansi_string_create("general") );
    ogl_shader                generation_fs   = ogl_shader_create               (_context, SHADER_TYPE_FRAGMENT, system_hashed_ansi_string_create("creation") );
    ogl_shader                modification_fs = ogl_shader_create               (_context, SHADER_TYPE_FRAGMENT, system_hashed_ansi_string_create("modification") );

    _generation_po   = ogl_program_create(_context, system_hashed_ansi_string_create("generation") );
    _modification_po = ogl_program_create(_context, system_hashed_ansi_string_create("modification") );

    ogl_shader_set_body(generation_fs,   system_hashed_ansi_string_create(fragment_shader_generation) );
    ogl_shader_set_body(modification_fs, system_hashed_ansi_string_create(fragment_shader_modification) );

    ogl_program_attach_shader(_generation_po, generation_fs);
    ogl_program_attach_shader(_generation_po, shaders_vertex_fullscreen_get_shader(general_vs) );

    ogl_program_attach_shader(_modification_po, modification_fs);
    ogl_program_attach_shader(_modification_po, shaders_vertex_fullscreen_get_shader(general_vs) );
    ...
    ogl_program_get_uniform_by_name(_generation_po,   system_hashed_ansi_string_create("time"),  &time_descriptor);
    ogl_program_get_uniform_by_name(_modification_po, system_hashed_ansi_string_create("input"), &input_descriptor);
    ...
    entry_points->pGLProgramUniform1i(ogl_program_get_id(_modification_po), _modification_po_input_location, 0);
}

/* Stage step 1: sample data generation */
static void _stage_step_generate_data(ogl_context context, system_timeline_time time, void* not_used)
{
    const ogl_context_gl_entrypoints_ext_direct_state_access* dsa_entry_points = NULL;
    const ogl_context_gl_entrypoints*                         entry_points     = NULL;
    uint32_t                                                  time_ms          = 0;
    ...
    system_time_get_msec_for_timeline_time(time, &time_ms);
    ...
    entry_points->pGLUniform1f (_generation_po_time_location,      time_ms_as_s);

    entry_points->pGLBindFramebuffer(GL_DRAW_FRAMEBUFFER, _fbo_id);
...
static void _stage_step_modify_data(ogl_context context, system_timeline_time time, void* not_used)
{
    ...
    entry_points->pGLUseProgram             (ogl_program_get_id(_generation_po) );
    dsa_entry_points->pGLBindMultiTextureEXT(GL_TEXTURE0,                       GL_TEXTURE_2D, _to_1);

    entry_points->pGLBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
    entry_points->pGLDrawArrays     (GL_TRIANGLE_FAN,     0, 4);
}
...
int WINAPI WinMain(HINSTANCE instance_handle, HINSTANCE, LPTSTR, int)
{
    ...
    ogl_pipeline_add_stage_step(_pipeline, _pipeline_stage_id, system_hashed_ansi_string_create("Data generation"), stage_1_callback, NULL);
    ogl_pipeline_add_stage_step(_pipeline, _pipeline_stage_id, system_hashed_ansi_string_create("Data update"),     stage_2_callback, NULL);
