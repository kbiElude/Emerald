...
/** TODO */
static void _stage_step_mesh_render(ogl_context          context,
                                    ...
                                    void*                not_used)
{
    ...
    mvp = system_matrix4x4_create_by_mul(projection_matrix,
                                         _stage_step_mesh_view_matrix);

    ogl_uber_set_shader_general_property(_stage_step_mesh_uber,
                                         OGL_UBER_GENERAL_PROPERTY_CAMERA_LOCATION,
                                         camera_location);
    ogl_uber_set_shader_general_property(_stage_step_mesh_uber,
                                         OGL_UBER_GENERAL_PROPERTY_VP,
                                         mvp);
    ogl_uber_set_shader_item_property   (_stage_step_mesh_uber,
                                         _uber_light_id,
                                         OGL_UBER_ITEM_PROPERTY_FRAGMENT_LIGHT_COLOR,
                                         light_color);
    ogl_uber_set_shader_item_property   (_stage_step_mesh_uber,
                                         _uber_light_id,
    ...
    ogl_uber_set_shader_item_property   (_stage_step_mesh_uber,
                                        ...
                                        &_stage_step_light_sh_data);

    ogl_uber_rendering_start(_stage_step_mesh_uber);
    {
        ogl_uber_rendering_render(_stage_step_mesh_uber,
                                  1,
                                 &_stage_step_mesh_model_matrix,
                                 &scene_mesh,
                                  scene);
    }
                                                        ...
                                                        __in           uint32_t     stage_id)
{
    ...
    _stage_step_mesh_uber = ogl_uber_create        (_stage_step_mesh_context,
                                                    system_hashed_ansi_string_create("Uber program") );
    _uber_light_id        = ogl_uber_add_light_item(_stage_step_mesh_uber,
                                                    diffuse_uber_light,
                                                    specular_uber_light,
                                                    0,
                                                    NULL);
    ...
    sh_samples samples = sh_samples_create(context,
                                           100 * 100,
                                           3 /* bands */,
                                           system_hashed_ansi_string_create("SH samples") );
    ...
    _stage_step_mesh_scene_projection_combiner = sh_projector_combiner_create(context,
                                                                              SH_COMPONENTS_RGB,
                                                                              samples,
                                                                              system_hashed_ansi_string_create("SH projection combiner") );

    sh_projector_combiner_add_sh_projection                 (_stage_step_mesh_scene_projection_combiner,
                                                             SH_PROJECTION_TYPE_CIE_OVERCAST,
                                                            &projection_id);
    sh_projector_combiner_get_sh_projection_property_details(_stage_step_mesh_scene_projection_combiner,
                                                             projection_id,
                                                             SH_PROJECTOR_PROPERTY_SKY_ZENITH_LUMINANCE,
                                                            &zenith_luminance_curve);

    system_variant_set_float         (temp_variant,
                                      .025f);
    curve_container_set_default_value(zenith_luminance_curve,
                                      temp_variant);

    /* Calculate SH projection coefficients */
    sh_projector_combiner_execute(_stage_step_mesh_scene_projection_combiner,
                                  0);
    ...
    ogl_pipeline_add_stage_step(pipeline,
                                stage_id,
                                system_hashed_ansi_string_create("Mesh rendering"),
                                _stage_step_mesh_render,
                                NULL);
}
