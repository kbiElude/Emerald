...
{
    stage_step_mesh_deinit     (context,
                                _pipeline);
    stage_step_particles_deinit(context,
                                _pipeline);
}
                              ...
                              system_variant result)
{
    system_variant_set_float(result,
                             stage_step_particles_get_decay() );
}
...
{
    system_variant_set_float(result,
                             stage_step_particles_get_gravity() );
}
...
{
    system_variant_set_float(result,
                             stage_step_particles_get_minimum_mass() );
}
...
{
    system_variant_set_float(result,
                             stage_step_particles_get_spread() );
}
...
PRIVATE void _init_gl(ogl_context context,
                      void*       arg)
{
    stage_step_mesh_init     (context,
                              _pipeline,
                              _stage_id);
    stage_step_particles_init(context,
                              _pipeline,
                              _stage_id);
}
                           ...
                           void* not_used2)
{
    ...
    ogl_ui_get_control_property(_ui_toggle_debug_checkbox,
                   ...
                   int)
{
    ...
    if (!ocl_get_platform_id(0,
                            &_cl_platform_id) )
    {
        LOG_FATAL("Cannot retrieve OpenCL platform ID - crash imminent");
    }

    _cl_context = ocl_context_create_gpu_only_with_gl_sharing(_cl_platform_id,
                                                              _gl_context,
                                                              system_hashed_ansi_string_create("CL context") );
    ...
    _pipeline = ogl_pipeline_create   (_gl_context,
                                       true,
    ...
    _ui_toggle_debug_checkbox = ogl_ui_add_checkbox(pipeline_ui,
                                                    system_hashed_ansi_string_create("Debug"),
                                                    button3_x1y1,
                                                    stage_step_particles_get_debug(),
                                                    _toggle_debug,
                                                    NULL);
    ...
    ogl_ui_add_scrollbar(pipeline_ui,
                         ...
                         system_variant_create_float(50.0f),
                         scrollbar_3_x1y1,
    ...
    ogl_flyby_activate(_gl_context,
                       camera_pos);

    ogl_flyby_set_property(_gl_context,
    ...
    /* Load the scene */
    _scene        = scene_load                    (_gl_context,
                                                   system_hashed_ansi_string_create("sphere_on_plane") );
    _scene_kdtree = ocl_kdtree_load               (_cl_context,
                                                   system_hashed_ansi_string_create("sphere_on_plane_kdtree"),
                                                   OCL_KDTREE_USAGE_MANY_LOCATIONS_ONE_RAY);
    _scene_mesh   = scene_get_unique_mesh_by_index(_scene,
                                                   0);
