...
static void _stage_step_background_execute(ogl_context context, system_timeline_time time, void* not_used)
{
    ...
    system_matrix4x4_set_from_matrix4x4(_inv_projection, projection_matrix);
    system_matrix4x4_invert            (_inv_projection);

    entrypoints->pGLBindFramebuffer(GL_DRAW_FRAMEBUFFER, _fbo_id);
    entrypoints->pGLDrawBuffers    (1, &draw_buffer);

    ogl_flyby_get_view_matrix(context, _mv);
    ogl_skybox_draw          (_skybox, NULL, _mv, _inv_projection);
}
...
PUBLIC void stage_step_background_init(ogl_context context, ogl_pipeline pipeline, uint32_t stage_id)
{
    ...
    _result_texture = ogl_texture_create(context, system_hashed_ansi_string_create("BG result texture") );

    dsa_entrypoints->pGLTextureImage2DEXT   (_result_texture, GL_TEXTURE_2D, 0,                     GL_RGBA32F, main_get_output_resolution()[0], main_get_output_resolution()[1], 0, GL_RGBA, GL_FLOAT, NULL);
    dsa_entrypoints->pGLTextureParameteriEXT(_result_texture, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);

    /* Initialize FBO */
    entrypoints->pGLGenFramebuffers(1, &_fbo_id);

    dsa_entrypoints->pGLNamedFramebufferTexture2DEXT(_fbo_id, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _result_texture, 0);

    /* Initialize skybox data */
    _skybox_image   = gfx_rgbe_load_from_file                       (system_hashed_ansi_string_create("galileo_probe.hdr") );
    _skybox_texture = ogl_texture_create_from_gfx_image             (context, _skybox_image,   system_hashed_ansi_string_create("skybox texture") );
    _skybox         = ogl_skybox_create_spherical_projection_texture(context, _skybox_texture, system_hashed_ansi_string_create("skybox") );

    /* Add ourselves to the pipeline */
    ogl_pipeline_add_stage_step(pipeline, stage_id, system_hashed_ansi_string_create("Background"), _stage_step_background_execute, NULL);
}
