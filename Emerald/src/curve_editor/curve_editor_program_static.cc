...
REFCOUNT_INSERT_IMPLEMENTATION(curve_editor_program_static, curve_editor_program_static, _curve_editor_program_static);
...
/** Please see header for specification */
PUBLIC curve_editor_program_static curve_editor_program_static_create(__in __notnull ogl_context context, __in __notnull system_hashed_ansi_string name)
{
    _curve_editor_program_static* result = new (std::nothrow) _curve_editor_program_static;

    ASSERT_DEBUG_SYNC(result != NULL, "Out of memroy while instantiating static program object.");
    if (result != NULL)
    {
        /* Reset the structure */
        ...
        REFCOUNT_INSERT_INIT_CODE_WITH_RELEASE_HANDLER(result, 
                                                       ...
                                                       system_hashed_ansi_string_create_by_merging_two_strings("\\Curve Editor Programs (Static)\\", system_hashed_ansi_string_get_buffer(name)) );
    }
