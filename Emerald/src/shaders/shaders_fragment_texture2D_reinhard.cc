...
const char* tex2D_fragment_reinhardt_shader_body_not_reverted = "#version 330\n"
                                                                ...
                                                                "void main()\n"
                                                                "{\n"
                                                                "    vec3 tex = texture2D(tex, uv).xyz * vec3(exposure);\n"
                                                                "\n"
...
const char* tex2D_fragment_reinhardt_shader_body_reverted = "#version 330\n"
                                                            ...
                                                            "void main()\n"
                                                            "{\n"
                                                            "    vec3 tex = texture2D(tex, vec2(uv.x, 1-uv.y)).xyz * vec3(exposure);\n"
                                                            "\n"
                                                            "    result = vec4(pow(tex / (1 + tex), vec3(1/2.2) ), 1);\n"
                                                            "}\n";
...
REFCOUNT_INSERT_IMPLEMENTATION(shaders_fragment_texture2D_reinhardt, shaders_fragment_texture2D_reinhardt, _shaders_fragment_texture2D_reinhardt);
...
PUBLIC EMERALD_API shaders_fragment_texture2D_reinhardt shaders_fragment_texture2D_reinhardt_create(__in __notnull ogl_context context, __in bool should_revert_y, __in __notnull system_hashed_ansi_string name)
{
    ...
    if (!ogl_shader_set_body(shader, system_hashed_ansi_string_create(should_revert_y ? tex2D_fragment_reinhardt_shader_body_reverted : tex2D_fragment_reinhardt_shader_body_not_reverted) ))
    {
    ...
    ASSERT_DEBUG_SYNC(result_object != NULL, "Out of memory while instantiating _shaders_fragment_texture2D_reinhardt object.");
    ...
    REFCOUNT_INSERT_INIT_CODE_WITH_RELEASE_HANDLER(result_object, 
                                                   ...
                                                   system_hashed_ansi_string_create_by_merging_two_strings("\\Texture2D Reinhardt Fragment Shaders\\", system_hashed_ansi_string_get_buffer(name)) );
