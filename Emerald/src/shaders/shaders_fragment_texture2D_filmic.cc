...
const char* tex2D_fragment_filmic_shader_body_not_reverted = "#version 330\n"
                                                             ...
                                                             "void main()\n"
                                                             "{\n"
                                                             "    vec3 tex = max(vec3(0.0), texture2D(tex, uv).xyz * vec3(exposure) - 0.004);\n"
                                                             "\n"
...
const char* tex2D_fragment_filmic_shader_body_reverted = "#version 330\n"
                                                         ...
                                                         "void main()\n"
                                                         "{\n"
                                                         "    vec3 tex = max(vec3(0.0), texture2D(tex, vec2(uv.x, 1 - uv.y) ).xyz * vec3(exposure) - 0.004);\n"
                                                             "\n"
                                                             "    result = vec4((tex * (6.2 * tex + 0.5)) / (tex * (6.2 * tex + 1.7) + 0.06), 1);\n"
                                                         "}\n";
 ...
 **/
PRIVATE void _shaders_fragment_texture2D_filmic_release(__in __notnull __deallocate(mem) void* ptr)
{
...
/** Please see header for specification */
PUBLIC EMERALD_API ogl_shader shaders_fragment_texture2D_filmic_get_shader(__in __notnull shaders_fragment_texture2D_filmic shader)
{
