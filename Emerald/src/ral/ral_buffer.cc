                                    ...
                                    const ral_buffer_create_info* create_info_ptr)
{
    ...
    if (create_info_ptr->is_sparse)
    {
        ASSERT_DEBUG_SYNC(false,
                          "Sparse buffer support is TODO");
    ...
    if (create_info_ptr->usage_bits >= (1 << (RAL_QUEUE_LAST_USED_BIT + 1)) )
    {
        ASSERT_DEBUG_SYNC(false,
    ...
    if (create_info_ptr->user_queue_bits >= (1 << (RAL_BUFFER_USAGE_LAST_USED_BIT + 1) ))
    {
        ASSERT_DEBUG_SYNC(false,
